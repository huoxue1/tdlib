...
func CheckUpdate(version string) string {
	...
	if versionCompare(version, latest) {
		log.Infof("当前有更新的 study_xxqg 可供更新, 请前往 https://github.com/huoxue1/tdlib/releases 下载.")
...
func LastVersion() (string, error) {
	response, err := http.Get("https://api.github.com/repos/huoxue1/tdlib/releases/latest")
	if err != nil {
		return "", err
	}
...
func binaryName() string {
	...
	return fmt.Sprintf("tdlib_%v_%v.%v", runtime.GOOS, goarch, ext)
}

func checksum(github, version string) []byte {
	sumURL := fmt.Sprintf("%v/huoxue1/tdlib/releases/download/%v/tdlib_checksums.txt", github, version)
...
// SelfUpdate 自更新
func SelfUpdate(github string, version string) error {
	if github == "" {
		github = "https://github.com"
	}

	url := fmt.Sprintf("%v/huoxue1/tdlib/releases/download/%v/%v", github, version, binaryName())
	log.Info("当前最新版本为 ", version)
	log.Info("正在下载,请稍等...")
	sum := checksum(github, version)
	if sum != nil {
		err := update(url, sum)
		if err != nil {
			log.Error("下载失败: ", err)
